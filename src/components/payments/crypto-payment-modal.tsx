...
import { Bitcoin, Loader2, Copy, CheckCircle2, AlertCircle, Clock, QrCode, ExternalLink, Info } from "lucide-react"
      ...
      const response = await fetch("/api/payments/coinbase/create", {
        ...
        body: JSON.stringify({
          ...
          description: productName
            ? `Payment for ${productName}${tierName ? ` - ${tierName}` : ""}`
            ...
            <>
              ...
              <div className="space-y-2">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-xs text-slate-500">(auto-generated by Coinbase)</span>
                </div>
                <div className="flex gap-2">
                  <input
                    ...
                    disabled
                    className="flex-1 px-3 py-2 bg-slate-900/60 border border-slate-700 rounded-lg text-sm text-slate-400 font-mono overflow-hidden cursor-not-allowed select-all"
                  />
                  <Button
                    variant="outline"
                    size="sm"
                ...
                <p className="text-xs text-slate-500 flex items-start gap-1">
                  ...
                  This is a one-time address generated for this payment. Do not reuse it for future transactions.
                </p>
              </div>
              ...
              <div className="space-y-3">
                ...
                {/* QR Code */}
                {state.cryptoDetails.address ? (
                  <div className="bg-white rounded-lg p-4 flex flex-col items-center gap-3 mx-auto w-fit">
                    <QRCodeSVG
                      value={state.cryptoDetails.address}
                      size={180}
                      level="H"
                  ...
                  <div className="bg-slate-900 border border-dashed border-slate-700 rounded-lg p-8 flex flex-col items-center gap-3">
                    ...
                    <p className="text-xs text-slate-500">No address available for QR code</p>
                  </div>
                )}
              </div>
